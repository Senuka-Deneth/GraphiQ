...
const RULE_ID = "cmp.provided-is-interface";

export const componentProvidedIsInterfaceRule: UmlRule = {
  id: RULE_ID,
  ...
  check(model: UmlModel): Diagnostic[] {
    ...
    for (const relationship of model.relationships) {
      if (relationship.relationshipType !== "interfaceRealization") {
        continue;
      }
      ...
      diagnostics.push({
        ...
        ruleId: RULE_ID,
        severity: "error",
        message: "Provided interface realization must go from a component to an interface",
