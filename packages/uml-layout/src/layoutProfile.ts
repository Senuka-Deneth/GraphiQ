import { CLASS_BOX, getElementNotation } from "@graphiq/uml-notation";
...
function isLayoutElement(element: UmlElement): boolean {
  return (
    ...
    element.elementType === "profile" ||
    element.elementType === "note"
  );
}

export function measureProfileNode(element: UmlElement): Pick<OverlayNode, "width" | "height"> {
  ...
  const keywordRows = notation.keyword !== undefined || element.elementType === "metaclass" ? 1 : 0;
  let bodyRows = 0;
  if (element.elementType === "stereotype") {
    bodyRows = element.attributes.length;
  } else if (element.elementType === "enumeration") {
    bodyRows = element.literals.length;
  }

  const height = Math.max(
    CLASS_BOX.minHeight,
    CLASS_BOX.nameCompartmentHeight + (keywordRows + bodyRows) * CLASS_BOX.rowHeight,
  );
...
function buildElkGraph(
  ...
  const children: ElkGraphInput[] = layoutElements.map((element) => {
    const measured = measureProfileNode(element);
...
export async function layoutProfile(
  model: UmlModel,
  overlay: NotationOverlay,
  mode: LayoutMode,
