...
import {
  useDocumentStore,
  type StateMachineStencilDropKind,
...
type StateMachineCanvasInnerProps = {
  onSelectedNodeChange?: (nodeId: string | null) => void;
};

function StateMachineCanvasInner({ onSelectedNodeChange }: StateMachineCanvasInnerProps) {
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      ) as StateMachineStencilDropKind;
      if (!kind) {
        return;
      }
  ...
  return (
    <div className="h-full w-full" data-testid="state-machine-canvas">
      <MarkerDefs />
      <ReactFlow
        nodes={nodes}
        edges={edges}
...
type StateMachineCanvasProps = {
  onSelectedNodeChange?: (nodeId: string | null) => void;
};

export function StateMachineCanvas({ onSelectedNodeChange }: StateMachineCanvasProps) {
  return (
    <ReactFlowProvider>
      <StateMachineCanvasInner onSelectedNodeChange={onSelectedNodeChange} />
    </ReactFlowProvider>
  );
}
