...
import {
  useDocumentStore,
  type CommunicationStencilDropKind,
...
type CommunicationCanvasInnerProps = {
  onSelectedNodeChange?: (nodeId: string | null) => void;
};

function CommunicationCanvasInner({ onSelectedNodeChange }: CommunicationCanvasInnerProps) {
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      ) as CommunicationStencilDropKind;
      if (!kind) {
        return;
      }
  ...
  const onNodeDoubleClick = useCallback(
    (_event: React.MouseEvent, node: Node) => {
      const currentName =
        ...
        node.data !== null &&
        "instanceName" in node.data &&
        typeof node.data.instanceName === "string"
  ...
  return (
    <div className="h-full w-full" data-testid="communication-canvas">
      <MarkerDefs />
      <ReactFlow
        nodes={nodes}
        edges={edges}
...
type CommunicationCanvasProps = {
  onSelectedNodeChange?: (nodeId: string | null) => void;
};

export function CommunicationCanvas({ onSelectedNodeChange }: CommunicationCanvasProps) {
  return (
    <ReactFlowProvider>
      <CommunicationCanvasInner onSelectedNodeChange={onSelectedNodeChange} />
    </ReactFlowProvider>
  );
}
