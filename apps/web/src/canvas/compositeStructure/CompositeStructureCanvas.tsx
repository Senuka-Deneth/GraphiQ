...
import {
  useDocumentStore,
  type CompositeStructureStencilDropKind,
...
type CompositeStructureCanvasInnerProps = {
  onSelectedNodeChange?: (nodeId: string | null) => void;
};

function CompositeStructureCanvasInner({
  onSelectedNodeChange,
}: CompositeStructureCanvasInnerProps) {
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      ) as CompositeStructureStencilDropKind;
      if (!kind) {
        return;
      }
  ...
  return (
    <div className="h-full w-full" data-testid="composite-structure-canvas">
      <MarkerDefs />
      <ReactFlow
        nodes={nodes}
        edges={edges}
...
type CompositeStructureCanvasProps = {
  onSelectedNodeChange?: (nodeId: string | null) => void;
};

export function CompositeStructureCanvas({
  onSelectedNodeChange,
}: CompositeStructureCanvasProps) {
  return (
    <ReactFlowProvider>
      <CompositeStructureCanvasInner onSelectedNodeChange={onSelectedNodeChange} />
    </ReactFlowProvider>
  );
}
