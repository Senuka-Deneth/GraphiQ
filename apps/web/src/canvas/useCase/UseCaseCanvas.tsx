...
import {
  useDocumentStore,
  type UseCaseStencilDropKind,
} from "../../store/documentStore.js";
import { ActorNode, actorNodeTypeName } from "./ActorNode.js";
import { SubjectNode, subjectNodeTypeName } from "./SubjectNode.js";
import { UseCaseNode, useCaseNodeTypeName } from "./UseCaseNode.js";
import { useCaseModelToFlow } from "./modelToFlow.js";

const nodeTypes = {
  [actorNodeTypeName]: ActorNode,
  [useCaseNodeTypeName]: UseCaseNode,
  [subjectNodeTypeName]: SubjectNode,
...
type UseCaseCanvasInnerProps = {
  onSelectedNodeChange?: (nodeId: string | null) => void;
};

function UseCaseCanvasInner({ onSelectedNodeChange }: UseCaseCanvasInnerProps) {
  ...
  const { nodes, edges } = useMemo(
    () => useCaseModelToFlow(model, overlay, diagnostics),
  ...
  return (
    <div className="h-full w-full" data-testid="use-case-canvas">
      <MarkerDefs />
      <ReactFlow
        nodes={nodes}
        edges={edges}
...
type UseCaseCanvasProps = {
  onSelectedNodeChange?: (nodeId: string | null) => void;
};

export function UseCaseCanvas({ onSelectedNodeChange }: UseCaseCanvasProps) {
  return (
    <ReactFlowProvider>
      <UseCaseCanvasInner onSelectedNodeChange={onSelectedNodeChange} />
    </ReactFlowProvider>
  );
}
